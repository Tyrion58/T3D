...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        # Use MASTER_PORT from env if available, otherwise fallback to 2333
        nccl_port = os.environ.get("MASTER_PORT", "2333")
        nccl_addr = os.environ.get("MASTER_ADDR", "127.0.0.1")
        print(f"[ModelRunner rank={rank}] Initializing NCCL on {nccl_addr}:{nccl_port}, world_size={self.world_size}", flush=True)
        dist.init_process_group("nccl", f"tcp://{nccl_addr}:{nccl_port}", world_size=self.world_size, rank=rank)
        print(f"[ModelRunner rank={rank}] NCCL initialized successfully", flush=True)
        torch.cuda.set_device(rank)
        default_dtype = torch.get_default_dtype()
        torch.set_default_dtype(hf_config.torch_dtype)
        torch.set_default_device("cuda")
        print(f"[ModelRunner rank={rank}] Creating model (type={hf_config.model_type})", flush=True)
        ...
        print(f"[ModelRunner rank={rank}] Loading model weights...", flush=True)
        load_model(self.model, config.model)
        print(f"[ModelRunner rank={rank}] Running warmup...", flush=True)
        # Sampler is removed from here
        self.warmup_model()
        print(f"[ModelRunner rank={rank}] Allocating KV cache...", flush=True)
        self.allocate_kv_cache()
        # CUDA graph capture for block diffusion is complex and omitted for this example
        if not self.enforce_eager:
            print(f"[ModelRunner rank={rank}] Capturing CUDA graphs...", flush=True)
            self.capture_cudagraph()
        print(f"[ModelRunner rank={rank}] Initialization complete", flush=True)
        torch.set_default_device("cpu")
        torch.set_default_dtype(default_dtype)

        if self.world_size > 1:
            # Use unique shm name per worker group based on MASTER_PORT to avoid conflicts
            # between multiple concurrent LLM instances
            master_port = os.environ.get("MASTER_PORT", "2333")
            shm_name = f"jetengineshm_{master_port}"
            if rank == 0:
    ...
    def allocate_kv_cache(self):
        ...
        print(f"[KV Cache] total={total/1e9:.1f}GB, free={free/1e9:.1f}GB, "
              f"peak={peak/1e9:.1f}GB, blocks={config.num_kvcache_blocks}, "
              f"block_size={self.block_size}", flush=True)
        assert config.num_kvcache_blocks > 0, f"Not enough memory for KV cache! free={free/1e9:.1f}GB"
